...
\item \code{readerControl}: a list with the named components
  \code{reader}, \code{language}, \code{load} and optionally
  \code{...}. A reader constructs a text document from a single element
...
\item \code{dbControl}: a list with the named components \code{useDb}
  ...
  \code{filehash} or \code{filehashSQLite}. Under activated database
