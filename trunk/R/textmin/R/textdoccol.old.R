          ...
          function(object, inputType = "PLAIN", stripWhiteSpace = FALSE, toLower = FALSE) {
              ...
              switch(type,
                     ...
                     "PLAIN" = {
                         ...
                         filenameIDs <- list(FileNames = filelist, IDs = as.character(1:length(filelist)))
                         tdl <- sapply(filelist,
                                       function(file, FileNameIDs = filenameIDs) {
                                           id <- FileNameIDs$IDs[grep(file, FileNameIDs$FileNames)]
                                           origin <- dirname(file)
                                           new("PlainTextDocument", FileName = file, Cached = FALSE, Author = "Unknown", DateTimeStamp = date(),
...
parseRISPlain <- function(file, stripWhiteSpace = FALSE, toLower = FALSE) {
    ...
    id <- paste(year, senat, number, sep = "")
...
parseReutersXML<- function(file) {
    new("XMLTextDocument", FileName = file, Cached = FALSE, Author = "REUTERS", DateTimeStamp = date(),
        Description = "Reuters21578 file containing several news articles", ID = "0",
