...
\class{GmaneRSource} and
\class{ReutersSource} which handle a directory, a mixed CSV, a Gmane R
...
\code{read\_gmane\_r} and \code{read\_newsgroup}). The default just
...
TextDocCol(DirSource(rcv1, load = TRUE), read_rcv1)
...
TextDocCol(DirSource(newsgroup, load = TRUE), read_newsgroup)
@
...
TextDocCol(GmaneRSource(rss), read_gmane_r)
@
...
heading and has an \code{ID} equal to 10 (both are metadata slot
...
with \code{append\_elem} and \code{append\_meta}, respectively.
<<>>=
data(crude)
reut21578.tdc <- append_elem(reut21578.tdc, crude[[1]], list())
...
Most standard operators are available for text document
