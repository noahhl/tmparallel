...
%% \VignetteIndexEntry{Introduction to the tmparallel Package}
...
data("crude")
@
\title{Introduction to the \pkg{tmparallel} Package\\Text Mining in \proglang{R}}
...
\pkg{tmparallel} package. We present methods for data import, corpus handling,
...
mining infrastructure offered by \pkg{tmparallel} was published in the
...
\section*{Data Import}
The main structure for managing documents in \pkg{tmparallel} is a so-called
...
object which abstracts the input location. \pkg{tmparallel} provides a set of
...
document from elements delivered by a source. The \pkg{tmparallel} package
...
reut21578 <- system.file("texts", "crude", package = "tmparallel")
...
e.g., stemming, stopword removal, et cetera. In \pkg{tmparallel}, all this
functionality is subsumed into the concept of a
\emph{transformation}. Transformations are done via the \code{tmparallel\_map()}
...
and \code{tmparallel\_map()} just applies them to all documents in a corpus.
...
given properties. For this purpose the function \code{tmparallel\_filter} is
...
\pkg{tmparallel} is to use the \code{meta()} function. A text document has a
...
meta data and \pkg{tmparallel} meta data structures and can be similarly used
...
For corpora the story is a bit more difficult. Corpora in \pkg{tmparallel}
...
from a corpus. In the \pkg{tmparallel} package the classes
