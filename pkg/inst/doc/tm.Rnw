...
fully in memory. Such a volatile corpus can be created via the
constructor \code{Corpus(x, readerControl)}. Another
implementation is the \class{PCorpus} which implements a
\emph{Permanent Corpus} semantics, i.e., the documents are physically
located on a disk, and its changes are reflected to all \proglang{R}
...
and the string ``\code{COMPUTER TERMINAL SYSTEMS <CPML> COMPLETES SALE}'' as
...
regular expressions:
<<>>=
tm_filter(reuters, pattern = "partnership")
@
...
few predefined attributes like \code{Author}, but can be extended with an
