...
is the so-called \class{VCorpus} (short for \emph{V}olatile
\emph{Corpus}) which realizes a semantics as known from most
\proglang{R} objects: corpora are \proglang{R} objects held fully in
memory. Such a volatile corpus can be created via the constructor
\code{Corpus(object, readerControl)}. Another implementation is the
\class{PCorpus} which implements a \emph{P}ermanent \emph{Corpus}
semantics, i.e., the documents are physically located on a disk, and
its changes are reflected to all \proglang{R} objects associated with
it.
...
For the corpus constructor, \code{readerControl} has to be a list with
the named components \code{reader} and \code{language}. The first
component \code{reader} constructs a text document from elements
delivered by a source. The \pkg{tm} package ships with several readers
(e.g., \code{readPlain()}, \code{readRCV1()},
\code{readReut21578XMLasPlain()}, \code{readGmane()},
\code{readNewsgroup()}, \code{readPDF()}, \code{readDOC()},
