...
@
\code{Content} contains the actual content of the
document, and \code{LocalMetaData} can be used to model arbitrary
additional user-defined meta data values.

So for our data frame we define a possible mappings as follows:
<<Mapping>>=
m <- list(Content = "contents", Heading = "title", Author = "authors", Topic = "topics")
@
...
vectors. The constructed reader will map each list entry to an attribute or
meta datum corresponding to the named list entry. Valid names include
\code{Content} to access the document's content, any valid attribute name
